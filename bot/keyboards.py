...
from .emoji import fallback, icon
...
def btn(
    ...
    Если задан icon_key, текст автоматически получает обычный эмодзи этого ключа,
    чтобы подпись и премиум-иконка не расходились.
    """
    label = f"{fallback(icon_key)} {text}" if icon_key else text
...
def main_menu(is_admin: bool = False) -> InlineKeyboardMarkup:
    ...
    if is_admin:
        rows.append([btn("Админка", "m:admin", ButtonStyle.DANGER, "gear")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def back_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[btn("Назад", "m:main", icon_key="back")]])
...
def history_nav(page: int, total_pages: int) -> InlineKeyboardMarkup:
    ...
    rows.append([btn("Назад", "m:main", icon_key="back")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def admin_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [btn("Назад", "m:main", icon_key="back")],
        ]
    )
